...
class ResContextBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut.replace_feature(self.act1(shortcut.features))
        shortcut.replace_feature(self.bn0(shortcut.features))

        shortcut = self.conv1_2(shortcut)
        shortcut.replace_feature(self.act1_2(shortcut.features))
        shortcut.replace_feature(self.bn0_2(shortcut.features))

        resA = self.conv2(x)
        resA.replace_feature(self.act2(resA.features))
        resA.replace_feature(self.bn1(resA.features))

        resA = self.conv3(resA)
        resA.replace_feature(self.act3(resA.features))
        resA.replace_feature(self.bn2(resA.features))
        resA.replace_feature(resA.features + shortcut.features)
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        upA.replace_feature(self.trans_act(upA.features))
        upA.replace_feature(self.trans_bn(upA.features))

        ## upsample
        upA = self.up_subm(upA)

        upA.replace_feature(upA.features + skip.features)

        upE = self.conv1(upA)
        upE.replace_feature(self.act1(upE.features))
        upE.replace_feature(self.bn1(upE.features))

        upE = self.conv2(upE)
        upE.replace_feature(self.act2(upE.features))
        upE.replace_feature(self.bn2(upE.features))

        upE = self.conv3(upE)
        upE.replace_feature(self.act3(upE.features))
        upE.replace_feature(self.bn3(upE.features))
...
class ReconBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut2.replace_feature(self.bn0_2(shortcut2.features))
        shortcut2.replace_feature(self.act1_2(shortcut2.features))

        shortcut3 = self.conv1_3(x)
        shortcut3.replace_feature(self.bn0_3(shortcut3.features))
        shortcut3.replace_feature(self.act1_3(shortcut3.features))
        shortcut.replace_feature(shortcut.features + shortcut2.features + shortcut3.features)

        shortcut.replace_feature(shortcut.features * x.features)
...
class Asymm_3d_spconv(nn.Module):
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        up0e.replace_feature(torch.cat((up0e.features, up1e.features), 1))
