...
def main(args):
    ...
    if os.path.exists(model_load_path):
        logging.info(f'Load mode from {model_load_path}')
    ...
    with torch.no_grad():
        for i_iter_demo, (_, demo_vox_label, demo_grid, demo_pt_labs, demo_pt_fea) in enumerate(
            ...
            for count, i_demo_grid in enumerate(demo_grid):
                i_d_grid = demo_grid[count]
                pl = predict_labels[
                    count, i_d_grid[:, 0], i_d_grid[:, 1],
                    i_d_grid[:, 2]] # max value 19
                t = fast_hist_crop(pl, demo_pt_labs[count], unique_label)
                hist_list.append(t)
                inv_labels = np.vectorize(inv_learning_map.__getitem__)(pl)
